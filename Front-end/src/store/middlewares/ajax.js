...
const ajaxMiddleware = store => next => (action) => {
  ...
  switch (action.type) {
    ...
    case REGISTER_USER: {
      ...
      axios.post('http://localhost/Apotheose/Project/sakura-cafe/Back-end/wp-json/wp/v2/users/register', { username: state.signup__username__input, email: state.signup__email__input, password: state.signup__password__input })
        .then((response) => {
